...
# 配置数据已提取到 arknights_sheet_config.py（无 PySide6 依赖）
from .arknights_sheet_config import (
    ARKNIGHTS_USER_CONTEXT_OVERRIDES,
    ARKNIGHTS_USER_VARS,
...
def create_arknights_compute_sheet(
    ...
    *,
    ...
    variables: dict[str, Any] = dict(dag_service.dag.variables)
...
def set_user_input_value(sheet: ComputeSheet, path: str, value: Any) -> None:
    ...
    entry = sheet._input_widgets.get(path)  # type: ignore[reportPrivateUsage]
    if entry is None:
        return
    widget, spec = entry
