...
from games.endfield.data_loading.curve_materialize import GROWTH_PARAM_KEY, materialize_weapon_entity
from games.endfield.data_loading.loader import CHARACTERS_JSON_PATH, WEAPONS_JSON_PATH
from utils.path_utils import get_resource_path

CHAR_REQUIRED = {"名称", "类型", "星级"}

WEAPON_REQUIRED = {"名称", "类型", "星级"}


def _weapon_has_attack_storage(weapon: dict) -> bool:
    if isinstance(weapon.get("基础攻击力"), list):
        return True
    growth = weapon.get(GROWTH_PARAM_KEY)
    return isinstance(growth, dict) and isinstance(growth.get("基础攻击力"), dict)


def _weapon_attack_curve(weapon: dict) -> list:
    if isinstance(weapon.get("基础攻击力"), list):
        return weapon["基础攻击力"]
    return materialize_weapon_entity(weapon)["基础攻击力"]
...
class TestGameDataContract(unittest.TestCase):
    ...
    def test_each_weapon_required_fields_and_curves(self):
        for weapon in self.weapons:
            ...
            self.assertTrue(
                _weapon_has_attack_storage(weapon),
                f"武器「{name}」须含 基础攻击力 数组或 成长参数.基础攻击力",
            )

            atk = _weapon_attack_curve(weapon)
            ...
            if isinstance(growth, dict):
                for key, entry in growth.items():
                    if isinstance(key, str) and key.endswith("+") and isinstance(entry, dict):
                        baked = materialize_weapon_entity(weapon).get(key)
                        if isinstance(baked, list):
                            self.assertEqual(
                                len(baked),
                                BONUS_ATTR_LEN,
                                f"武器「{name}」成长参数附加属性「{key}」烘焙后长度应为 {BONUS_ATTR_LEN}",
                            )
