...
class TestSearchHistoryDialog:
    def test_create_empty(self) -> None:
        _app()
        with patch("gui_design.controls.search.qt_search_browser.scan_search_output", return_value=[]):
    ...
    def test_create_with_db(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_list_runs(p: Path) -> list:
                from gui_design.controls.search.qt_search_browser import RunInfo
            ...
            with (
                patch("gui_design.controls.search.qt_search_browser.scan_search_output", side_effect=fake_scan),
                patch("gui_design.controls.search.qt_search_browser.list_runs", side_effect=fake_list_runs),
                patch("gui_design.controls.search.qt_search_browser.list_scores", side_effect=fake_list_scores),
            ):
                dialog = SearchHistoryDialog(big_font=MagicMock(), small_font=MagicMock())
