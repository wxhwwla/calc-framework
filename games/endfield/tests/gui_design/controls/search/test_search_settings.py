...
class TestGetCpuParallelInfo:
    def test_with_explicit_count(self) -> None:
        ...
        assert info.recommended_workers == 7
...
class TestDefaultParallelWorkers:
    def test_explicit_count(self) -> None:
        assert default_parallel_workers(cpu_count=4) == 3
...
class TestResolveParallelWorkers:
    def test_auto_choice(self) -> None:
        assert resolve_parallel_workers("自动 (7 线程)", cpu_count=8) == 7

    def test_auto_prefix(self) -> None:
        assert resolve_parallel_workers("自动", cpu_count=4) == 3

    def test_numeric_choice(self) -> None:
        assert resolve_parallel_workers("4", cpu_count=8) == 4

    def test_capped_by_max(self) -> None:
        assert resolve_parallel_workers("16", cpu_count=4) == 4

    def test_minimum_one(self) -> None:
        assert resolve_parallel_workers("0", cpu_count=4) == 1

    def test_empty_choice(self) -> None:
        assert resolve_parallel_workers("", cpu_count=4) == 3

    def test_garbage_text(self) -> None:
        assert resolve_parallel_workers("not a number", cpu_count=4) == 3


class TestFormatParallelWorkersHelp:
    def test_contains_core_info(self) -> None:
        info = CpuParallelInfo(logical_cores=8, recommended_workers=7, max_workers=8)
...
class TestFormatSearchProgressText:
    def test_preparing_when_total_zero(self) -> None:
        text = format_search_progress_text(
            prefix="搜索", processed=0, total=0, eta_seconds=0,
        )
        assert "准备中" in text

    def test_with_eta(self) -> None:
        text = format_search_progress_text(
            prefix="搜索", processed=50, total=100, eta_seconds=30,
        )
    ...
    def test_with_estimated_total(self) -> None:
        text = format_search_progress_text(
            prefix="搜索", processed=50, total=100,
            eta_seconds=30, estimated_total_seconds=120,
        )
        assert "总预计" in text
        assert "2 分钟" in text
    ...
    def test_no_eta(self) -> None:
        text = format_search_progress_text(
            prefix="搜索", processed=50, total=100, eta_seconds=0,
        )
        assert "50/100" in text
