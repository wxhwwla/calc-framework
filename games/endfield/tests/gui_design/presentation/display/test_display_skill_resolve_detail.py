...
from gui_design.presentation.display.skill_resolve import (
...
class TestResolveSelectedSkillForDamage:
    def test_skill_1_selected(self) -> None:
        result = resolve_selected_skill_for_damage(
            _make_char_data(), skill_1_level=1
        )
    ...
    def test_level_too_high_falls_through(self) -> None:
        result = resolve_selected_skill_for_damage(
            _make_char_data(), skill_1_level=99
        )
        assert "默认" in result.label
