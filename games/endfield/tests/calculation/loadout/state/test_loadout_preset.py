...
class TestLoadoutPreset(unittest.TestCase):
    ...
    def test_enemy_params_roundtrip(self) -> None:
        preset = LoadoutPreset(
            char_name="测试干员",
            weapon_name="测试武器",
            char_level=80,
        ...
        restored = import_preset_json(export_preset_json(preset))
        self.assertAlmostEqual(restored.merged_enemy_params()["enemy_defense"], 250.0)
        self.assertEqual(restored.merged_enemy_params()["combo_stacks"], 3)
        self.assertAlmostEqual(restored.merged_enemy_params()["attached_effect_multiplier"], 1.33)
        self.assertEqual(restored.merged_enemy_params()["enemy_tier"], "精英")
