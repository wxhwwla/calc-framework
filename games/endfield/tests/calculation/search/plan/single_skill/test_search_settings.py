...
class TestSearchSettings(unittest.TestCase):
    def test_resolve_parallel_workers_auto_and_numeric(self):
        ...
        self.assertEqual(resolve_parallel_workers("4", cpu_count=8), 1)

        self.assertEqual(resolve_parallel_workers("99", cpu_count=8), 1)
    ...
    def test_cpu_parallel_info_and_help_text(self):
        ...
        self.assertEqual(info.recommended_workers, 1)
        ...
        self.assertIn("逻辑线程", help_text)

        self.assertIn("搜索 worker", help_text)
