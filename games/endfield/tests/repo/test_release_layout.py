...
class TestReleaseLayout(unittest.TestCase):
    ...
    def test_target_app_names(self):
        self.assertEqual(target_app_name("launcher"), "Game Calc Platform")

    def test_release_readmes_mention_exe_version(self):
        from release_bundle.release_layout import _launcher_readme

        text = _launcher_readme(exe_version="0.5.0", package_version="1.18.0")
        self.assertIn("EXE v0.5.0", text)
        self.assertIn("Game Calc Platform", text)

    def test_stage_release_folder_copies_json_and_licenses_launcher(self):
        repo_root = REPO_ROOT
        app_name = target_app_name("launcher")
        with tempfile.TemporaryDirectory() as tmp:
            release_root = Path(tmp) / app_name
            stage_release_folder(release_root, project_root=REPO_ROOT, repo_root=repo_root, target="launcher")
    ...
    def test_release_dir_from_dist(self):
        ...
        self.assertEqual(
            release_dir_from_dist(dist_dir, target="launcher"),
            dist_dir / "Game Calc Platform",
        )
