...
def combat_multiplier_field_name(display_field: str) -> str:
    """面板倍率字段 → 可选实战倍率字段（G44）。"""
    text = str(display_field or "").strip()
    if text.endswith("倍率"):
        return f"{text[:-2]}实战倍率"
    return f"{text}实战倍率"
...
def segment_multipliers_at_level(
    char_data: dict[str, Any],
    field_name: str,
    *,
    skill_level: int,
    ...
    """返回 (面板倍率, 实战倍率)；无实战字段时实战与面板相同。"""
    display = segment_multiplier_at_level(
        char_data,
        field_name,
    ...
    if display is None:
        return None, None
    combat_field = combat_multiplier_field_name(field_name)
    combat_raw = segment_multiplier_at_level(
        char_data,
        combat_field,
        skill_level=skill_level,
        segment_index=segment_index,
    )
    combat = combat_raw if combat_raw is not None else display
    return display, combat
...
def segment_display_label(
    ...
    *,
    ...
    combat_multiplier_percent: float | None = None,
    damage_type_display: str | None = None,
) -> str:
    """GUI 行标签，如 ``连携技 第2段 (400%)`` 或 ``战技 第1段 (350%→348%)``。"""
    ...
    if multiplier_percent is None:
        ...
        if (
            ...
            base = f"{skill_type} 第{seg}段 ({display_text})"
            ...
            specs.append(
                {
                    ...
                    "label": segment_display_label(
                        key,
                        multiplier_percent=display_pct,
                        combat_multiplier_percent=combat_pct,
