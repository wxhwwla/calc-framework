...
def _make_ability_bonus_subgraph() -> DAGSubgraph:
    """能力值加成子图 = 主能力平值×0.005 + 副能力平值×0.002。"""
    return DAGSubgraph(
        ...
        nodes={
            ...
            "n_main": B(op="*", lhs="main_flat", rhs="n_main_mul", label="主能力*(1+主能力%/100)"),
            "n_sub_pct_div": B(op="/", lhs="sub_pct", rhs="const_100"),
            "n_sub_mul": B(op="+", lhs="const_1", rhs="n_sub_pct_div"),
            "n_sub": B(op="*", lhs="sub_flat", rhs="n_sub_mul", label="副能力*(1+副能力%/100)"),
            "n_main_x_0005": B(op="*", lhs="n_main", rhs="const_0005"),
            "n_sub_x_0002": B(op="*", lhs="n_sub", rhs="const_0002"),
            "result": B(op="+", lhs="n_main_x_0005", rhs="n_sub_x_0002", label="能力值加成 = main*0.005 + sub*0.002"),
        },
        outputs={
...
def _make_final_attack_subgraph() -> DAGSubgraph:
    ...
    return DAGSubgraph(
        ...
        nodes={
            "const_1": C(value=1.0),
            "n_base_atk": B(op="+", lhs="char_base_atk", rhs="weapon_base_atk", label="基础攻击力 = 角色+武器"),
            "n_add_one": B(op="+", lhs="const_1", rhs="atk_bonus"),
            "n_atk_bonus_part": B(op="*", lhs="n_add_one", rhs="n_base_atk", label="攻击加成攻击力 = 基础*(1+攻击力+)"),
            "n_add_atk_equip": B(op="+", lhs="additional_atk", rhs="equip_flat_atk"),
            "n_mid_atk": B(
                op="+", lhs="n_atk_bonus_part", rhs="n_add_atk_equip", label="中间攻击力 = 攻击加成攻击力+附加+装备"
            ),
            "n_ability_one": B(op="+", lhs="const_1", rhs="ability_bonus"),
            "result": B(op="*", lhs="n_mid_atk", rhs="n_ability_one", label="最终攻击力 = 中间*(1+能力值加成)"),
        },
        outputs={
...
def _make_single_hit_damage_subgraph() -> DAGSubgraph:
    ...
    return DAGSubgraph(
        ...
        nodes={
            "result": B(op="*", lhs="final_attack", rhs="skill_mult", label="单段伤害 = 最终攻击力 * 技能倍率"),
        },
        outputs={
...
def _make_crit_zone_subgraph() -> DAGSubgraph:
    ...
    return DAGSubgraph(
        ...
        nodes={
            ...
            "result": B(op="+", lhs="const_1", rhs="n_crit_part", label="暴击区 = 1 + 暴击率*(暴击伤害-1)"),
        },
        outputs={
...
def _make_base_damage_block_subgraph() -> DAGSubgraph:
    ...
    return DAGSubgraph(
        ...
        nodes={
            "n_single_hit": B(op="*", lhs="final_attack", rhs="skill_mult", label="单段基础伤害"),
            "result": B(op="*", lhs="n_single_hit", rhs="crit_zone", label="暴击后伤害 = 单段基础 * 暴击区"),
        },
        outputs={
...
def _make_buff_debuff_block_subgraph() -> DAGSubgraph:
    """增益减益块子图：7 个乘区（伤害加成/减免/增幅/虚弱/庇护/脆弱/易伤）连乘。"""
    return DAGSubgraph(
        ...
        nodes={
            ...
            "result": B(op="*", lhs="damage_after_crit", rhs="mult", label="增益减益后伤害"),
        },
        outputs={
...
def _make_environment_block_subgraph() -> DAGSubgraph:
    """环境乘区块子图：防御减伤 × 失衡易伤 × 抗性 × 非主控减伤 × 连击增伤 × 特殊乘区。"""
    return DAGSubgraph(
        ...
        nodes={
            ...
            "defense_reduction": B(op="/", lhs="const_100", rhs="n_def_add", label="防御减伤 = 100/(100+防)"),
            ...
            "env_mult": B(op="*", lhs="defense_reduction", rhs="n_env_3", label="环境 6 乘区连乘"),
            "result": B(op="*", lhs="damage_after_buff", rhs="env_mult", label="最终伤害"),
        },
        outputs={
