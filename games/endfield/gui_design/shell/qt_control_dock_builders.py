...
class BuilderMixin:
    ...
    def _build_col_multi(self) -> QWidget:
        ...
        lay.addWidget(HintLabel("按异常类型与等级填入触发次数。", self._small))
        ...
        self._physical_abnormal_widget, self._physical_abnormal_edits = build_abnormal_matrix(
        ...
        self._spell_abnormal_widget, self._spell_abnormal_edits = build_abnormal_matrix(
    ...
    def rebuild_segment_rows(self, char_data: dict | None, s1: int, s2: int, s3: int) -> None:
        """按角色技能段规格重建段级次数输入行。"""
        from calc_engine.endfield.calc.skills.segments import list_segment_count_specs
