...
DAG_PATH = Path(__file__).resolve().parent.parent / "framework" / "src" / "calc_framework" / "configs" / "endfield_full.dag.json"
...
def validate_layout(layout: dict[str, Any], dag: dict[str, Any], attr_schema: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for i, section in enumerate(sections):
        ...
        if stype not in _VALID_SECTION_TYPES:
            issues.append({"severity": "error", "section_id": sid, "field": "type", "message": f"无效 section type: {stype!r}"})
        ...
        if stype == "inputs":
            ...
            if not variables:
                issues.append({"severity": "warning", "section_id": sid, "field": "variables", "message": "input section 没有 variables"})
            for v in variables:
                ...
                if v in dag_variables:
                    ...
                    if not isinstance(var_def, dict) or "source" not in var_def:
                        issues.append({"severity": "warning", "section_id": sid, "field": "variables", "message": f"变量 {v!r} 定义不完整"})
                else:
                    short_name = v.split(".", 1)[1] if "." in v else v
                    if short_name in attr_names:
                        continue
                    issues.append({"severity": "error", "section_id": sid, "field": "variables", "message": f"变量 {v!r} 既不在 DAG variables 中，也不在 attr_schema 或 user_input 中"})

        if stype == "outputs":
            outputs = section.get("outputs", [])
            if not outputs:
                issues.append({"severity": "warning", "section_id": sid, "field": "outputs", "message": "output section 没有 outputs"})
            for o in outputs:
                referenced_outputs.add(o)
                if o not in dag_outputs:
                    issues.append({"severity": "error", "section_id": sid, "field": "outputs", "message": f"输出 {o!r} 不在 DAG outputs 中"})

        if stype == "widget":
            widget_type = section.get("widget_type", "")
            if not widget_type:
                issues.append({"severity": "warning", "section_id": sid, "field": "widget_type", "message": "widget section 缺少 widget_type"})
    ...
    if used_vars < total_vars:
        ...
        issues.append({"severity": "info", "message": f"layout 未覆盖 {len(uncovered)}/{total_vars} 个 DAG variables: {', '.join(sorted(uncovered)[:10])}"})

    if used_outputs < total_outputs:
        uncovered = set(dag_outputs.keys()) - referenced_outputs
        issues.append({"severity": "info", "message": f"layout 未覆盖 {len(uncovered)}/{total_outputs} 个 DAG outputs: {', '.join(sorted(uncovered)[:10])}"})
...
def main() -> None:
    ...
    if use_json:
        ...
        print(f"  Section 数: {result['stats']['sections']} (inputs={result['stats']['inputs_sections']}, outputs={result['stats']['outputs_sections']}, widget={result['stats']['widget_sections']})")
