...
def _extract_character_segments(
    raw: Dict[str, Any], mf: str, tf: str
) -> List[SegmentSchema]:
...
def _make_segment(
    rate_list: List[Any], raw_types: Any, seg_index: int
) -> SegmentSchema:
...
def _read_damage_type(type_field_value: Any, index: int) -> Optional[str]:
    ...
    for v in raw_list:
        ...
        try:
            ...
            if fv == int(fv) and fv < 100:
                ...
                result.append(int(round(fv * 100)))
            else:
                result.append(int(round(fv)))
