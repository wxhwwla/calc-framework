...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-i", "--input", default=None, help="源截图文件夹（省略则弹对话框）")

    parser.add_argument("-o", "--output", default=None, help="数据集根目录（默认: ./终末地数据集）")

    parser.add_argument("--stats", action="store_true", help="只统计已有数据集，不做整理")
...
def _collect_images(
    src: Path, dst_root: Path, extensions: tuple[str, ...] = (".png", ".jpg", ".jpeg", ".bmp", ".webp")
) -> int:
    """交互式分类整理截图。"""
...
def _print_stats(root: Path) -> None:
    ...
    for cat_dir in sorted(root.iterdir()):
        ...
        images = [f for f in cat_dir.iterdir() if f.suffix.lower() in (".png", ".jpg", ".jpeg", ".bmp", ".webp")]
