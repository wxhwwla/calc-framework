...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "-i", "--input", type=str, default=None, help="输入路径：图片文件或文件夹（省略则弹对话框选择）"
    )

    parser.add_argument("-m", "--model", type=str, default=None, help="YOLO 模型 .pt 文件路径（省略则用 YOLOv8n.pt）")

    parser.add_argument("-o", "--output", type=str, default=None, help="输出目录（省略则在输入目录下创建 _detected）")

    parser.add_argument("--conf", type=float, default=0.25, help="置信度阈值 (default: 0.25)")

    parser.add_argument("--iou", type=float, default=0.45, help="IoU 阈值 (default: 0.45)")

    parser.add_argument("--device", type=str, default="cpu", help='推理设备 "cpu" 或 "cuda:0" (default: cpu)')

    parser.add_argument("--no-annotated", action="store_true", help="不保存标注图片")

    parser.add_argument("--no-json", action="store_true", help="不保存 JSON 检测结果")
