...
打包脚本 - 支持多目标构建。
使用方法:
    python scripts/build.py                           # 打包计算器（默认）
    python scripts/build.py --target calculator       # 同上
    python scripts/build.py --target designer         # 构建设计器
    python scripts/build.py --target layout-editor    # 构建布局编辑器
...
看门狗（可选环境变量）:
  ENDFIELD_BUILD_TIMEOUT_SECONDS  默认 1200 (20 分钟)
  ENDFIELD_BUILD_HEARTBEAT_SECONDS  默认 15
"""
    ...
    *,
    ...
    while True:
        ...
        if now - last_heartbeat >= heartbeat_seconds:
            print(f"[build] 仍在执行... 已用时 {elapsed}s", flush=True)
            last_heartbeat = now
