...
from games.endfield.calc.damage.formula import calculate_bonus_attribute, calculate_growth_curve

from games.endfield.calc.damage.inverse import fit_attribute_formula, fit_skill_formula_no_special
...
def _fit_conditional_special(rank_curves: dict[str, list[float]], p: dict[str, str], sub: str) -> dict[str, Any] | None:
    ...
    slot3_context = "\n".join(
        (p.get(key) or "").strip() for key in ("词条3内容", "满级词条3内容") if (p.get(key) or "").strip()
    )

    max_stack = _parse_max_stack_from_text(f"{raw_text}\n{slot3_context}", name=name)
...
def fit_weapon_base_atk_from_endpoints(
    ...
    *,
    ...
    if reference_curve and len(reference_curve) >= 90:
        ...
        scaled = [level1 + (ref[i] - ref[0]) * (level90 - level1) / denom for i in range(90)]

        sample = [round(v) for v in scaled]

    else:
        sample = [round(level1 + (level90 - level1) * (lv - 1) / 89) for lv in range(1, 91)]
...
def build_weapon_seed_spec_from_wiki(
    *,
    ...
    spec: dict[str, Any] = {
        ...
        "star": meta["star"],
        "base_atk": fit_weapon_base_atk_from_endpoints(l1, l90, reference_curve=ref_atk),
        "bonus_attrs": {},
