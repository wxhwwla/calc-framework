...
_SEED_PATH = Path(__file__).resolve().parent.parent.parent / "games" / "endfield" / "scripts" / "seed_characters.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"[{mode}] 计划处理 {len(result['planned'])} 人（更新 {len(result['updated'])}，新增 {len(result['added'])}）："
    )
