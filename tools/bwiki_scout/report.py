...
def write_summary_report(
    ...
    *,
    ...
    for kind, block in manifest.get("kinds", {}).items():
        lines.append(
            f"- **{kind}**：图鉴 {block.get('gallery_count', 0)}，分类补全 {block.get('category_count', 0)}，合并后 {block.get('merged_count', 0)}"
        )
...
def write_stats_diff_report(reports_dir: Path, stats: dict[str, Any]) -> Path:
    ...
    for row in stats.get("operators") or []:
        ...
        lines.append(f"### {row['name']}（{row['mismatch_count']} 处 / 对比 {row.get('compared_points', 0)} 点）")
