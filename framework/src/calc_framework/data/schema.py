...
def validate_variables(graph: DAGGraph, context: dict[str, Any]) -> None:
    ...
    for var_path, var_def in graph.variables.items():
        ...
        if value is None:
            raise VariableValidationError(f"变量 {var_path!r} 未在上下文中找到且无默认值")
...
def _check_type(var_path: str, value: Any, expected: str) -> None:
    ...
    if expected == "float" and not isinstance(value, int | float):
        raise VariableValidationError(f"变量 {var_path!r} 类型不匹配: 期望 float, 实际 {type(value).__name__}")

    if expected == "int":
        if isinstance(value, bool):
            raise VariableValidationError(f"变量 {var_path!r} 类型不匹配: 期望 int, 实际 bool")

        if not isinstance(value, int):
            raise VariableValidationError(f"变量 {var_path!r} 类型不匹配: 期望 int, 实际 {type(value).__name__}")

    if expected == "bool" and not isinstance(value, bool):
        raise VariableValidationError(f"变量 {var_path!r} 类型不匹配: 期望 bool, 实际 {type(value).__name__}")

    if expected == "str" and not isinstance(value, str):
        raise VariableValidationError(f"变量 {var_path!r} 类型不匹配: 期望 str, 实际 {type(value).__name__}")
