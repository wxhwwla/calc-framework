...
def _validate(data: dict[str, Any]) -> None:
    ...
    for sec in data["sections"]:
        ...
        if sec_type not in VALID_SECTION_TYPES:
            raise LayoutValidationError(f"section type 必须为 {'/'.join(VALID_SECTION_TYPES)}，收到: {sec_type}")
        ...
        if sec_type == "widget":
            ...
            if widget_type not in VALID_WIDGET_TYPES:
                raise LayoutValidationError(
                    f"widget section {section_id} 的 widget_type 必须为 {'/'.join(VALID_WIDGET_TYPES)}，收到: {widget_type}"
                )
