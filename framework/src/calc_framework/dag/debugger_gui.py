...
if _HAS_PYSIDE:
    ...
    class StepDebuggerWidget(_QtWidgets.QWidget):  # type: ignore[no-redef]
        ...
        def _setup_ui(self) -> None:
            self.setWindowTitle(tr("desktop.debugger.windowTitle"))
            self.resize(700, 500)
            ...
            self._btn_step = _QtWidgets.QPushButton(tr("desktop.debugger.stepBtn"))  # type: ignore[reportGeneralTypeIssues]
            self._btn_step.clicked.connect(self._on_step)
            btn_layout.addWidget(self._btn_step)

            self._btn_run = _QtWidgets.QPushButton(tr("desktop.debugger.runBtn"))  # type: ignore[reportGeneralTypeIssues]
            self._btn_run.clicked.connect(self._on_run)
            btn_layout.addWidget(self._btn_run)

            self._btn_reset = _QtWidgets.QPushButton(tr("common.reset"))  # type: ignore[reportGeneralTypeIssues]
            self._btn_reset.clicked.connect(self._on_reset)
            btn_layout.addWidget(self._btn_reset)

            layout.addLayout(btn_layout)

            self._status_label = _QtWidgets.QLabel(tr("desktop.debugger.progress", done=0, total=0))  # type: ignore[reportGeneralTypeIssues]
        ...
        def _on_step(self) -> None:
            if self._debugger.finished:
                self._status_label.setText(tr("desktop.debugger.finished"))
                return
            result = self._debugger.step()
            if result is None:
                self._status_label.setText(tr("desktop.debugger.allDone"))
                return
        ...
        def _update_node_display(self, node_id: str, value: float, is_bp: bool) -> None:
            for i, item in enumerate(self._node_items):
                if item.node_id == node_id:
                    ...
                    self._list_widget.item(i).setText(f"{icon} {item.label} = {value}  [{item.node_type}]")
        ...
        def _update_status(self) -> None:
            ...
            self._status_label.setText(tr("desktop.debugger.progressStatus", done=done, total=total, pct=pct, bp=bp_count))
    ...
    class StepDebuggerWidget:  # type: ignore[no-redef]
        def __init__(self, *args: object, **kwargs: object) -> None:
            _ = args, kwargs
            raise ImportError("PySide6 未安装。请执行: pip install calc-framework[ui]")
