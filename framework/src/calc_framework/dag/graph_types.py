...
def _parse_node(raw: dict[str, Any]) -> NodeType:
    ...
    if typ == "unary":
        ...
        return UnaryNode(op=op, input=_parse_ref(inp), label=label, description=desc)  # type: ignore[arg-type]
    ...
    if typ == "binary":
        ...
        return BinaryNode(op=op, lhs=_parse_ref(lhs), rhs=_parse_ref(rhs), label=label, description=desc)  # type: ignore[arg-type]
