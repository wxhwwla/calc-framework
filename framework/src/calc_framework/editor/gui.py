...
class _SectionWidget(QGroupBox):
    def __init__(
        ...
        type_label = tr("desktop.editor.input") if section_type == "inputs" else tr("desktop.editor.output")

        header.addWidget(QLabel(tr("desktop.editor.typePrefix", type=type_label)))
        ...
        if section_type == "inputs":
            ...
            layout.addWidget(QLabel(tr("desktop.editor.variables")))
            ...
            layout.addWidget(QLabel(tr("desktop.editor.outputs")))
...
class LayoutEditorWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        open_btn = QPushButton(tr("desktop.editor.loadDag"))

        open_btn.clicked.connect(self._open_dag)

        toolbar.addWidget(open_btn)

        export_btn = QPushButton(tr("desktop.editor.exportLayout"))

        export_btn.clicked.connect(self._export)

        toolbar.addWidget(export_btn)

        preview_btn = QPushButton(tr("desktop.editor.previewSheet"))
        ...
        left_layout.addWidget(QLabel(tr("desktop.editor.availableInputs")))

        self._input_list = QListWidget()

        left_layout.addWidget(self._input_list)

        left_layout.addWidget(QLabel(tr("desktop.editor.availableOutputs")))
        ...
        center_layout.addWidget(QLabel(tr("desktop.editor.sectionList")))

        add_btn_row = QHBoxLayout()

        self._section_title_input = QLineEdit()

        self._section_title_input.setPlaceholderText(tr("desktop.editor.sectionTitlePlaceholder"))

        add_btn_row.addWidget(self._section_title_input)

        add_inputs_btn = QPushButton(tr("desktop.editor.addInputSection"))

        add_inputs_btn.clicked.connect(lambda: self._add_section("inputs"))

        add_btn_row.addWidget(add_inputs_btn)

        add_outputs_btn = QPushButton(tr("desktop.editor.addOutputSection"))
        ...
        bottom.addWidget(QLabel(tr("desktop.editor.layoutName")))

        self._name_input = QLineEdit(tr("desktop.editor.defaultLayout"))
    ...
    def _open_dag(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, tr("desktop.editor.selectDagDialog"), "", "JSON Files (*.json);;All Files (*)"
        )
        ...
        try:
            ...
            QMessageBox.critical(self, tr("common.error"), tr("desktop.editor.cannotLoadDag", error=e))
    ...
    def _load_dag(self, dag: DAGGraph, _path: str = "") -> None:
        ...
        self._status_label.setText(tr("desktop.editor.loadedDag", name=dag.name or tr("common.default")))
    ...
    def _add_section(self, sec_type: str) -> None:
        ...
        if not title:
            title = tr("desktop.editor.inputSection") if sec_type == "inputs" else tr("desktop.editor.outputSection")
    ...
    def _export(self) -> None:
        if not self._editor:
            QMessageBox.warning(self, tr("desktop.editor.warning"), tr("desktop.editor.loadDagFirst"))

            return

        self._editor.set_name(self._name_input.text().strip() or tr("desktop.editor.defaultLayout"))

        path, _ = QFileDialog.getSaveFileName(self, tr("desktop.editor.exportDlgTitle"), "layout.json", "JSON Files (*.json)")
        ...
        try:
            ...
            self._status_label.setText(tr("desktop.editor.exportedMsg", path=path))

        except Exception as e:
            QMessageBox.critical(self, tr("desktop.editor.exportFailed"), str(e))
    ...
    def _preview(self) -> None:
        ...
        preview.setWindowTitle(tr("desktop.editor.previewTitle", name=layout.name))
        ...
        try:
            ...
            preview_layout.addWidget(QLabel(tr("desktop.editor.renderFailed", error=e)))
    ...
    def load_layout_file(self, dag_path: str, layout_path: str) -> None:
        ...
        self._status_label.setText(tr("desktop.editor.loadedLayout", name=editor.state.layout_name))
