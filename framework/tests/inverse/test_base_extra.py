...
class TestFloorFormulaFitterComputeDecimal:
    ...
    def test_compute_decimal(self):
        ...
        assert len(result) == 5
        # ×10 缩放：sb=100, sg=10; lv=2: (100+floor(10/2))/10=10.5
        assert result == [10.0, 10.5, 11.0, 11.5, 12.0]
...
class TestGcdNormalize:
    ...
    def test_gcd_reduction(self):
        ...
        scaled_base = 100
        growth, divisor, offset = FloorFormulaFitter._gcd_normalize(10, 2, 0, scaled_data, scaled_base)
        assert growth == 5
        assert divisor == 1
    ...
    def test_gcd_non_reducible(self):
        ...
        scaled_base = 100
        growth, divisor, offset = FloorFormulaFitter._gcd_normalize(3, 1, 0, scaled_data, scaled_base)
        assert growth == 3
        assert divisor == 1
    ...
    def test_gcd_offset_nonzero(self):
        ...
        scaled_base = 100
        growth, divisor, offset = FloorFormulaFitter._gcd_normalize(12, 2, 0, scaled_data, scaled_base)
        assert growth == 6
        assert divisor == 1
        assert offset == 0
    ...
    def test_gcd_break_early(self):
        ...
        scaled_base = 100
        growth, divisor, offset = FloorFormulaFitter._gcd_normalize(10, 4, 1, scaled_data, scaled_base)
        assert growth == 10
        assert divisor == 4
        assert offset == 1
