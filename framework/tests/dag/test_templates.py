...
class TestTemplateRegistration:
    def test_register_and_list(self):
        register_template("test_tpl", parameters=["x"],
                          nodes={"out": {"type": "const", "value": 42}},
                          output_node="out")
        assert "test_tpl" in list_templates()

    def test_register_twice_raises(self):
        register_template("dup", parameters=[], nodes={"out": {"type": "const", "value": 1}},
                          output_node="out")
        with pytest.raises(TemplateError, match="已注册"):
            register_template("dup", parameters=[], nodes={"out": {"type": "const", "value": 2}},
                              output_node="out")

    def test_register_empty_name_raises(self):
        with pytest.raises(TemplateError, match="模板名无效"):
            register_template("", parameters=[], nodes={"n": {"type": "const", "value": 1}},
                              output_node="n")
    ...
    def test_register_invalid_output_node_raises(self):
        with pytest.raises(TemplateError, match="不在 nodes 中"):
            register_template("bad", parameters=[], nodes={"a": {"type": "const", "value": 1}},
                              output_node="b")

    def test_unregister(self):
        register_template("t", parameters=[], nodes={"n": {"type": "const", "value": 1}},
                          output_node="n")
...
class TestExpandTemplateRefs:
    ...
    def test_expand_simple_template(self):
        register_template("double", parameters=["input_val"],
                          nodes={
    ...
    def test_multiple_template_instances(self):
        register_template("pass", parameters=["val"],
                          nodes={"out": {"type": "var", "path": "$val"}},
                          output_node="out")
        raw = {
            "nodes": {
    ...
    def test_template_label_propagates(self):
        register_template("labeled", parameters=["v"],
                          nodes={"out": {"type": "var", "path": "$v"}},
                          output_node="out")
        raw = {
            "nodes": {
    ...
    def test_unknown_binding_raises(self):
        register_template("simple", parameters=["x"],
                          nodes={"out": {"type": "const", "value": 1}},
                          output_node="out")
    ...
    def test_edge_binding_references_external(self):
        register_template("ref", parameters=["src"],
                          nodes={"out": {"type": "var", "path": "$src"}},
                          output_node="out")
        raw = {
            "nodes": {
    ...
    def test_inputs_dict_binding(self):
        register_template("expr_tpl", parameters=["a", "b"],
                          nodes={
                              "out": {"type": "expr", "expr": "a + b",
                                       "inputs": {"a": "$a", "b": "$b"}}
                          },
                          output_node="out")
        raw = {
    ...
    def test_template_plus_regular_nodes(self):
        dag_json = {
            ...
            "nodes": {
                ...
                "double_check": {
                    "type": "binary", "op": "*", "lhs": "def_reduc", "rhs": "atk_val"
                },
            },
            "outputs": {
