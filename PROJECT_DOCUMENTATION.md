...
> **维护说明**：日常开发、安装与测试请以 `endfield_damage_calculator/README.md` 为准；领域术语见仓库根目录 `CONTEXT.md`；版本号仅在 `please_read_me.py` 修改（`pyproject.toml` 自动同步 `_VERSION`）。本文档侧重算法与历史模块说明。
