...
| **固定配装** | GUI 底栏可固定 0–4 件具体装备（`FixedLoadoutSelection`）；未勾选部位在装备范围内遍历 |
| **计算与搜索区** | GUI **底栏**中部：武器/装备范围、固定配装、全量遍历、并行线程、预估与 MVP |
| **多技能次数区** | GUI **底栏**右侧：战技/连携/终结次数与「使用手动次数」开关 |
| **右侧乘区** | GUI 第 4 列（`ZONE_COLUMN`），展示 15 乘区链 |
| **角色属性列** | GUI 第 2 列，角色曲线与技能倍率 |
| **武器属性列** | GUI 第 3 列，武器攻击与附加属性 |
| **主界面列权重** | `(0, 0, 1, 1, 5)` + 底栏 `CONTROL_DOCK_ROW` 横跨左四列 |
