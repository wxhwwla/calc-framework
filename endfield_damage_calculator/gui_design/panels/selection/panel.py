...
class ChooseTypesStarsNamesLevels(
    ...
    def __init__(
        self, frame: ctk.CTkFrame, list_c_w: list[dict[str, Any]], my_font: ctk.CTkFont, is_weapon_panel: bool = False
    ):
        """
        初始化配置
        ...
        # 子组件
        self.trust_panel: TrustPanel | None = None  # 信赖面板（角色专用）
        self.skill_level_panel: SkillLevelPanel | None = None  # 技能等级面板（角色专用）
        self.special_ability_panel: SpecialAbilityPanel | None = None  # 特殊能力面板（武器专用）
        # 角色/武器侧默认展开「技能等级 / 武器技能」
        self._show_advanced_params_var: ctk.BooleanVar = ctk.BooleanVar(value=True)
        self._advanced_section_title: str = "武器技能" if is_weapon_panel else "技能等级"
    ...
    @classmethod
    def use(
        cls, frame: ctk.CTkFrame, list_c_w: list[dict[str, Any]], my_font: ctk.CTkFont, is_weapon_panel: bool = False
