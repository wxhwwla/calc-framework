...
from gui_design.layout.gui_layout import MORE_SETTINGS_VIEWPORT_HEIGHT
...
class TestEnhancementIntegration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        from gui_design.controls import enhancement as ec
    ...
    def test_startup_page_mode_selector_updates_preferences(self) -> None:
        ...
        with patch("gui_design.controls.enhancement.section.save_ui_preferences") as mock_save:
            self._ec.place_enhancement_section(self.app, parent, start_row=0, place_fn=place_fn)
            self.app._more_settings_toggle_btn.invoke()
    ...
    @patch("gui_design.controls.enhancement.dialogs.messagebox")
    def test_dashboard_without_snapshot_refreshes(self, mock_mb: MagicMock) -> None:
        with patch("gui_design.controls.enhancement.dialogs.is_matplotlib_available", return_value=True):
            with patch("matplotlib.use"):
                with patch(
                    "gui_design.controls.enhancement.dialogs.build_damage_pie_figure",
                    return_value=MagicMock(),
                ):
                    with patch(
                        "gui_design.controls.enhancement.dialogs.build_improvement_bar_figure",
                        return_value=MagicMock(),
                    ):
                        with patch(
    ...
    @patch("gui_design.controls.enhancement.dialogs.filedialog.askopenfilenames", return_value=())
    ...
    def test_preset_compare_with_two_json_files(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "gui_design.controls.enhancement.dialogs.filedialog.askopenfilenames",
                return_value=(str(p1), str(p2)),
            ):
                with patch("gui_design.controls.enhancement.dialogs.ctk.CTkToplevel") as mock_top:
                    mock_top.return_value = MagicMock()
                    with patch("gui_design.controls.enhancement.dialogs.ctk.CTkScrollableFrame"):
                        with patch("gui_design.controls.enhancement.dialogs.ctk.CTkLabel"):
                            self._ec.show_preset_compare_dialog(self.app)
    ...
    def test_history_dialog_empty(self) -> None:
        with patch("gui_design.controls.enhancement.dialogs.ctk.CTkToplevel") as mock_top:
            ...
            with patch("gui_design.controls.enhancement.dialogs.ctk.CTkScrollableFrame"):
                with patch("gui_design.controls.enhancement.dialogs.ctk.CTkLabel") as mock_label:
