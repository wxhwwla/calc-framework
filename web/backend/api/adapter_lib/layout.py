...
from api.adapter_lib.assets import get_adapter_dag, get_adapter_layout
from api.internal.json_utils import ADAPTER_ROOT, load_json
...
def _validate_layout(
    ...
    for i, section in enumerate(sections):
        ...
        if stype == "inputs":
            ...
            if not variables:
                issues.append(
                    ValidationIssue(
                        severity="warning", section_id=sid, field="variables", message="input section 没有 variables"
                    )
                )
            for v in variables:
                ...
                if v in dag_variables:
                    ...
                    if not isinstance(var_def, dict) or "source" not in var_def:
                        issues.append(
                            ValidationIssue(
                                severity="warning", section_id=sid, field="variables", message=f"变量 {v!r} 定义不完整"
                            )
                        )
                else:
        ...
        if stype == "outputs":
            ...
            if not outputs:
                issues.append(
                    ValidationIssue(
                        severity="warning", section_id=sid, field="outputs", message="output section 没有 outputs"
                    )
                )
            for o in outputs:
                ...
                if o not in dag_outputs:
                    issues.append(
                        ValidationIssue(
                            severity="error", section_id=sid, field="outputs", message=f"输出 {o!r} 不在 DAG outputs 中"
                        )
                    )
    ...
    if used_vars < total_vars:
        ...
        preview = ", ".join(sorted(uncovered)[:10])
        issues.append(
            ValidationIssue(
                severity="info",
                message=(f"layout 未覆盖 {len(uncovered)}/{total_vars} 个 DAG variables: {preview}"),
            )
        )
    ...
    if used_outputs < total_outputs:
        ...
        issues.append(
            ValidationIssue(
                severity="info",
                message=(f"layout 未覆盖 {len(uncovered)}/{total_outputs} 个 DAG outputs: {preview}"),
            )
        )
