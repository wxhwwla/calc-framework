...
| CustomTkinter | MIT | GUI 框架 |
| pytest 等开发依赖 | 各自开源许可 | 仅开发/测试使用 |

发布前请根据 `pyproject.toml` / 锁定依赖重新核对许可证列表。
...
| MediaWiki API（`scripts/bwiki_scout/`） | BWIKI 运营方 | 只读侦察；遵守访问频率与 robots/服务条款 |

**商标：**「明日方舟」「终末地」及相关标识为权利人商标。本仓库为爱好者工具，**未经权利人书面许可，不得暗示官方背书或商业合作**。
