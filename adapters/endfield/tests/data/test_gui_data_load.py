...
from adapters.endfield.data_loading.loader import DataLoadError, fetch_game_data_for_gui
...
class TestGuiDataLoad(unittest.TestCase):
    ...
    @patch("adapters.endfield.data_loading.game_data_facade.get_equipments", return_value=[])
    @patch("adapters.endfield.data_loading.game_data_facade.get_characters", return_value=[{"名称": "测试"}])
    @patch("adapters.endfield.data_loading.game_data_facade.get_weapons", return_value=[{"名称": "武器"}])
    ...
    def test_facade_create_matches_fetch_lists(self) -> None:
        with (
            patch("adapters.endfield.data_loading.game_data_facade.get_characters", return_value=[{"名称": "A"}]),
            patch("adapters.endfield.data_loading.game_data_facade.get_weapons", return_value=[{"名称": "W"}]),
            patch("adapters.endfield.data_loading.game_data_facade.get_equipments", return_value=[]),
        ):
