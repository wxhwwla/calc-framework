...
用法::
    ...
    python scripts/import_calcpack.py                    # GUI 模式（文件对话框）
    python scripts/import_calcpack.py path/to/game.calcpack  # CLI 模式
    python scripts/import_calcpack.py path/to/game.calcpack --dir custom/path  # 指定目标
"""
...
ADAPTERS_DIR = Path(__file__).resolve().parents[1] / "framework" / "adapters"
