...
> **测试基线**（2026-05-23）：`[包]` 下 `python -m pytest tests/ -q` → **276 passed**。
...
│   ├── loadout_slot_search.py # 遍历装备件数 1–4
...
主窗口 **6 列** grid，`APP_COLUMN_WEIGHTS = (0, 0, 1, 1, 1, 5)`（见 `gui_design/gui.py`）：

| 列 | 内容 | 伸缩 |
|----|------|------|
| 0–1 | 角色 / 武器选择 | 固定最小宽（约 260px） |
| 2 | **计算与搜索**（单段伤害、全量遍历、并行线程、预估） | 可伸缩，min 360px |
| 3–4 | 角色 / 武器属性列 | 可伸缩 |
| 5 | **右侧乘区**（15 乘区链展示） | 主伸缩列 |

启动最大化（`utils/gui_window.py`）；字体与系统一致（`utils/gui_fonts.py`）。确认选择后刷新属性列；角色与武器均有效时才更新乘区。启动时自动确认一次。

模块职责：
- `gui.py` — 主窗口、搜索控件、全量/MVP 按钮
- `selection_panel.py` / `selection_components.py` — 角色/武器选择（含技能等级）
- `property_display.py` — 确认后渲染属性列与乘区
- `preview_lines.py` — 单/多技能快速预览文案（纯函数）
- `gui_settings.py` — 主题；`search_export_paths.py` — `search_output/` 路径

---

### 5. 单技能全量搜索流水线

与 GUI「全量遍历」「MVP 导出/续跑」对应，核心约定见 [`会话接续手册.md`](会话接续手册.md) §2.2。
...
GUI 确认态
  ...
  → run_search_session (search_session.py)
       ...
       └─ 否则 → run_enumerated_optimizer_parallel + parallel_search.run_bounded_parallel
  → evaluate_task (loadout_optimizer.py)：装备属性词条重算 final_attack + 15 乘区伤害
```
...
| `equipment_prune.py` | 按主/副能力与目标技能类型**排序**（不减少组合总数） |
| `loadout_slot_search.py` | `varying_slot_count` 1–4：哪些格参与笛卡尔积 |
| `loadout_optimizer.py` | `build_optimizer_search_plan`、预计组合数、`evaluate_task` |
| `search_output/` | 续跑 SQLite、TopN 导出（gitignore） |

**注意**：改 `varying_slot_count`、剪枝或词条逻辑后应新建 run；勿用旧 `search_runs.db` 的 `total_combinations` 对比进度。
...
支持：角色/武器/装备选择、乘区展示、单段伤害、单技能全量遍历（1–4 件装备）、多技能快速预览（按次数加权）。详见 [`endfield_damage_calculator/README.md`](../endfield_damage_calculator/README.md) 与 [`MVP搜索验收说明.md`](MVP搜索验收说明.md)。
