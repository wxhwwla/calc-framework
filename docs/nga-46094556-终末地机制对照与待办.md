...
| **G8** | 部分 ✅ | `special_damage.py` + 真实伤害 GUI + 处决/治疗对话框生命汲取 |
| **G9** | 部分 ✅ | `consumable_presets.py` + 手动 buff 窗「消耗品预设」 |

### 4.2 异常与状态（P1）

| ID | 机制 | 现状 |
|----|------|------|
| **G10** | 部分 ✅ | 破防易伤 + `physical_abnormal_state.py` 层消耗/异常键区分 |
...
| **G21** | 部分 ✅ | 处决/治疗对话框：失衡上限/节点 + 单次累积估算 |
...
| ID | 机制 | 说明 |
|----|------|------|
| **G50** | 部分 ✅ | `combat_resources.py` + 处决/治疗对话框技力/终结充能估算 |
| **G51** | 部分 ✅ | 同上（6.5/100 技力、连携 +10、返还不计充能） |
| **G52** | 部分 ✅ | `enemy_growth.py` + 插件 `enemy_max_hp`；不硬算成长曲线 |
