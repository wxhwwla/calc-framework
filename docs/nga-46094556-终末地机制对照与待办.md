...
| **G8** | ✅ 2026-06-01 | `special_damage.py` + 真实伤害 GUI + 处决/治疗对话框/Web API |
| **G9** | ✅ 2026-06-01 | `consumable_presets.py` + GUI/Web 消耗品预设参考 |

### 4.2 异常与状态（P1）

| ID | 机制 | 现状 |
|----|------|------|
| **G10** | ✅ 2026-06-01 | 破防逐 hit 轮转：`iter_rotation_hits` + 快照/多技能搜索加权 |
| **G11** | ✅ 2026-06-01 | 法术异常键前缀 ``强制:`` → 无初始伤 |
...
| **G21** | ✅ 2026-06-01 | 处决/治疗/失衡对话框 + Web `POST /api/survival/estimate` |
...
| ID | 机制 | 现状 |
|----|------|------|
| **G40** | ✅ 2026-06-01 | `display_corrections.py` 附表扩展（攻/防/源石） |
| **G41–G43** | ✅ 2026-06-01 | 防御 21/28/36、源石 70/24/29/41、%12.3 校正 |
| **G44** | ✅ 2026-06-01 | `segments.py` 可选 `*实战倍率` 字段；计算用实战、标签双显 |

### 4.6 战斗循环（P3，偏模拟器）

| ID | 机制 | 说明 |
|----|------|------|
| **G50** | ✅ 2026-06-01 | `combat_resources.py` + 处决/治疗/Web 生存估算 |
| **G51** | ✅ 2026-06-01 | 同上（6.5/100 技力、连携 +10、返还不计充能） |
| **G52** | ✅ 2026-06-01 | `enemy_growth.py` + 插件 `enemy_max_hp`；不硬算成长曲线 |

### 4.7 产品与文档

| ID | 项 | 现状 |
|----|-----|------|
| **G60** | Web 终末地页 | ✅ MVP：敌参/搜索/快照/对比/生存估算/消耗品预设参考 |
| **G61** | ✅ 2026-06-01 | F1 说明书「NGA 机制对照」节 + 链至仓库文档 |
| **G62** | 会话手册 §3 | ✅ 2026-06-01 维护者已更新接缝表 |
...
| 2026-06-01 | 第八批 G9/G10 接线、G50–51 估算、消耗品预设 GUI、生命汲取 |
| 2026-06-01 | 第九批 G10 状态 API、G52 插件生命、G12/G21 对话框、搜索/签名补全 |
| 2026-06-01 | 第十批 `EnemyEvalParams` 预览全敌参、多技能预览破防轮转说明、G25 对话框 |
| 2026-06-01 | 第十一批 Web 搜索/预估/流式三处 `build_search_job_inputs_from_request`、snapshot/compare 敌参扩展、预览异常分项 `abnormal_eval_kwargs`、逐 hit 破防 API |
| 2026-06-01 | 第十二批 G60 Web 敌参面板（真实伤害/连击/破防/附带倍率/腐蚀）、搜索/快照/对比 payload 接线、`GET /enemies` 插件列表 |
| 2026-06-01 | 终批 G10 破防逐 hit 轮转、G21/G50 Web 生存估算 API、消耗品预设 API、手册 §3 接缝更新 |
